...
{
    // Заполняем производные только во внутренних узлах (индексы 2 … n-3)
    for (int i = 2; i <= n - 3; ++i)
    {
        m[i] = (f[i + 1] - f[i - 1]) / (x[i + 1] - x[i - 1]);
    }
    // Остальные значения m не используются, можно оставить неинициализированными
}
...
{
    if (n < 4) return 0.0;   // кубическая интерполяция не определена
